...
const MemberForm: React.FC<MemberFormProps> = ({ defaultValues, isEditing , onSubmitSuccess, handleCloseEditModal}) => {
  ...
  const onSubmit: SubmitHandler<Member> = async data => {
    ...
    const url = isEditing ? `https://172.16.5.78/api/update_member/${defaultValues.project_id}/${defaultValues.member?.member_id}` : `https://172.16.5.78/api/add_members/${defaultValues.project_id}`;
